...
regardless of motion sequencing. Resolves the arrival-moment problem in
Feature Context.
...
- AC1: By the time the visitor scrolls or otherwise interacts with the
  ...
  headline/tagline, ornamental mark, and scroll cue — together.
- AC2: The composition is never presented, delivered, or usable in a
  ...
  (e.g. no valid arrival state exists with the mark shown but no scroll
  cue).
- AC3: Reaching completeness is never conditional on, or delayed by, the
  weight/complexity of any individual visual element (e.g. the
  ornamental-mark asset).

**Validation Scenarios**

- Scenario A — Standard arrival
  - Success Condition: at the moment the visitor first scrolls or
    interacts, headline/tagline, ornamental mark, and scroll cue are all
    present.
  - Failure Condition: the visitor scrolls or interacts while one or more
    of the three elements has not yet appeared.
- Scenario B — Heavier visual element present
  - Success Condition: even when the ornamental mark (or another Hero
    visual element) is heavier/slower to render, the composition still
    reaches completeness before the visitor can scroll or interact.
  - Failure Condition: completeness is delayed or skipped because of that
...
- Scenario — Visitor activates the scroll cue
  - Success Condition: no scroll or navigation occurs as a direct result
    of that activation.
...
- AC1: For each device class the visitor's viewport falls into, the Hero
  composition presents a treatment appropriate to that device class.
- AC2: The composition on a smaller device class is not an unmodified
  reflow (simple scaling/wrapping) of the layout used on a larger device
  class.
